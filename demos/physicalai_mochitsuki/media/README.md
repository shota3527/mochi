# PhysicalAI Mochitsuki Media - eric_v3

Shared reference media for PR https://github.com/shota3527/mochi/pull/1.

## Videos

- `videos/latest_g1_mochitsuki_multi_angle_demo.mp4`: latest/best multi-camera render.
- `videos/latest_g1_mochitsuki_single_camera_demo.mp4`: latest single-camera render.
- `videos/v3_g1_mochitsuki_multi_angle_demo.mp4`: frozen v3 multi-camera render.
- `videos/v2_g1_mochitsuki_demo.mp4`: v2 render.

## Photos

- `photos/latest/multi_camera_keyframes/`: latest keyframes from all validation cameras.
- `photos/latest/camera_contact_sheets/`: latest per-camera contact sheets.
- `photos/latest/main_camera_keyframes/`: latest main-camera keyframes.
- `photos/latest/diagnostic_dense/`: latest dense diagnostic sheets.
- `photos/v3/`: v3 contact sheets and dense diagnostic sheets.

## Notes

- Code branch: `ekkkkki/mochi:eric_v3`.
- Upstream PR: https://github.com/shota3527/mochi/pull/1.
- The v2 and v3 trajectories are simulation/visual prototypes. Their dynamic warnings are documented in the version manifests in the PR.
